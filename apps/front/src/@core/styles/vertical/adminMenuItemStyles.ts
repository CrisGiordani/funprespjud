...
const adminMenuItemStyles = (verticalNavOptions: VerticalNavState, theme: Theme): MenuItemStyles => {
  // Vars
  ...
  return {
    root: ({ level }) => ({
      ...
      [`&.${menuClasses.subMenuRoot}.${menuClasses.open} > .${menuClasses.button}, &.${menuClasses.subMenuRoot} > .${menuClasses.button}.${menuClasses.active}`]:
        {
          backgroundColor: '#004F75 !important'
        },
    ...
    subMenuContent: ({ level }) => ({
      ...
      backgroundColor: popoutCollapsed ? '#1a1a1a' : 'transparent',
      ...(popoutCollapsed &&
        level === 0 && {
...
export default adminMenuItemStyles
